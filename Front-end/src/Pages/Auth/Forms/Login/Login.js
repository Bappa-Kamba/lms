...
const handleSuccess = (response) => {
  // 💡 IMPORTANT: The response now contains the JWT credential
  console.log("Login Success:", response.credential);

  // 1. Get the JWT (tokenId)
  const tokenId = response.credential;

  // 2. Send the JWT to your backend endpoint for verification
  fetch("http://localhost:8080/google_auth", {
    // or google_login
    method: "POST",
    headers: {
    ...
    .then((data) => {
      ...
      // Handle successful login/signup from your Express server
    })
    .catch((error) => {
